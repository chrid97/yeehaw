...
// include: /tmp/tmpoqalcdq8.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'build/yeehaw.data';
      var REMOTE_PACKAGE_BASE = 'yeehaw.data';
...
Module['FS_createPath']("/", "assets", true, true);
      ...
      async function processPackageData(arrayBuffer) {
        ...
        // Reuse the bytearray from the XHR as the source for file reads.
          ...
          Module['removeRunDependency']('datafile_build/yeehaw.data');
      }
      Module['addRunDependency']('datafile_build/yeehaw.data');
...
// end include: /tmp/tmpoqalcdq8.js
// include: /tmp/tmpcy5mhjfr.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpcy5mhjfr.js
// include: /tmp/tmpfszj9esh.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpfszj9esh.js
...
var ASM_CONSTS = {
  811912: () => { if (document.fullscreenElement) return 1; },  
 811958: () => { return Module.canvas.width; },  
 811990: () => { return parseInt(Module.canvas.style.width); },  
 812038: () => { document.exitFullscreen(); },  
 812065: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 812138: () => { if (document.fullscreenElement) return 1; },  
 812184: () => { return Module.canvas.width; },  
 812216: () => { return screen.width; },  
 812241: () => { document.exitFullscreen(); },  
 812268: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 812401: () => { return window.innerWidth; },  
 812427: () => { return window.innerHeight; },  
 812454: () => { if (document.fullscreenElement) return 1; },  
 812500: () => { return Module.canvas.width; },  
 812532: () => { return parseInt(Module.canvas.style.width); },  
 812580: () => { if (document.fullscreenElement) return 1; },  
 812626: () => { return Module.canvas.width; },  
 812658: () => { return screen.width; },  
 812683: () => { return window.innerWidth; },  
 812709: () => { return window.innerHeight; },  
 812736: () => { if (document.fullscreenElement) return 1; },  
 812782: () => { return Module.canvas.width; },  
 812814: () => { return screen.width; },  
 812839: () => { document.exitFullscreen(); },  
 812866: () => { if (document.fullscreenElement) return 1; },  
 812912: () => { return Module.canvas.width; },  
 812944: () => { return parseInt(Module.canvas.style.width); },  
 812992: () => { document.exitFullscreen(); },  
 813019: ($0) => { Module.canvas.style.opacity = $0; },  
 813057: () => { return screen.width; },  
 813082: () => { return screen.height; },  
 813108: () => { return window.screenX; },  
 813135: () => { return window.screenY; },  
 813162: () => { return window.devicePixelRatio; },  
 813198: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 813251: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 813302: () => { Module.canvas.style.cursor = 'none'; },  
 813339: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 813595: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 813646: () => { if (document.pointerLockElement) return 1; },  
 813693: () => { if (document.fullscreenElement) return 1; },  
 813739: () => { return window.innerWidth; },  
 813765: () => { return window.innerHeight; },  
 813792: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; let miniaudio = window.miniaudio; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === miniaudio.device_state.started) { device.webaudio.resume().then(() => { _ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 815970: () => { if (typeof(window.miniaudio) !== 'undefined') { miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 816260: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 816364: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 819412: ($0) => { return window.miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 819485: ($0) => { var device = window.miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 819885: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 819935: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 820074: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; }
};
